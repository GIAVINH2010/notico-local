              ...
              switch (options.mode) {
                ...
                case 'range':
                  ...
                  if (!options.lastSel) {
                    ...
                    options.date[0] = (tmp.setHours(0, 0, 0, 0)).valueOf();
                    console.log(options.addDay)
                    var deb = moment(options.date[0]).subtract(options.addDay, 'days').toDate()
                    //var info_deb = { month: deb.month(), year: deb.year(), day: deb.date() }
                    //console.log(info_deb)
                    var fin = moment(options.date[0]).add(options.addDay, 'days').toDate()
                   // var info_fin = { month: fin.month(), year: fin.year(), day: fin.date() }
                   // console.log(info_fin)
                    options.selectableDates = [deb, fin]
                    
                } else {
                  options.selectableDates = null
                }
